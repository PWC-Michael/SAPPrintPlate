            ...
            <b-row>
                <b-col sm="12">
                    <b-card>
                        ...
                        <div v-if="!contactsLoaded" class="form-container">
                            <LoadingSpinner />
                        </div>
                        <div v-if="contactsLoaded" class="form-container">
                            <b-row>
...
const getContacts = (($http, companyId) => {
    return new Promise((resolve, reject) => {
        let headersObj = getHTTPHeaders();
        $http.get('getContact?company_id=' + companyId, headersObj)
            .then((response) => {
    ...
    getContacts() {
        getCurrentUser(this.$http)
            .then((userDetails) => {
                ...
                return getContacts(this.$http, userDetails.company_id);
            })
            .then((contactList) => {
                console.log(contactList);
                this.$data.tableTotalRows = contactList.length;
                this.$data.contactItems = contactList;
                this.$data.contacts = contactList;
                this.$data.contactsLoaded = true;
            })
            .catch ((err) => {
                console.error(err);
            });
